...
Router.onBeforeAction(requireLogin, {except:['registrationNotReady', ,'frontPage','aboutPage','registerAccount','privacy','terms-of-use']});
Router.onBeforeAction(requireStaff,{only:['runnerRegistrationSummary','officialRaceTime',,'raceResults','stoppedRunnersByNumber','stoppedRunnersAdultMale','stoppedRunnersAdultFemale','stoppedRunnersHSMale','stoppedRunnersHSFemale','stoppedRunnersMSMale','stoppedRunnersMSFemale','stoppedRunnersLSMale','stoppedRunnersLSFemale','payment','paymentConfirmationFrontPage']});


Router.map(function() {
       this.route('frontPage',{path:'/'});
        this.route('registerAccount',{path:'/account/'})
     this.route('registrationForm', {path: '/registration-temp/'});
        this.route('userPortal',{path:'/portal/'})
        this.route('aboutPage',{path:'/about/'})
     this.route('registrationNotReady',{path:'/registration/'});
    this.route('adminTemplate',{path:'/admin/',
                               onBeforeAction:requireStaff});
    ...
    waitOn: function(){
     ...
     return Meteor.subscribe('runners',{runnerEmail:Meteor.user().emails[0].address},{});    
      ...
      this.route('privacy',{path:'/account/privacy'});
